...
s2 = s2.filter(ee.Filter.bounds(dusaf))
    .filterDate(start, end)
    .select(['B7', 'B8', 'B8A', 'B10']);
// S2Cloudless for the cloud probability band.
s2c = s2c.filterDate(start, end).filter(ee.Filter.bounds(dusaf));
// S2 L2A for surface reflectance bands.
s2Sr = s2Sr.filterDate(start, end).filter(ee.Filter.bounds(dusaf))
    .select(['B2', 'B3', 'B4', 'B5']);
